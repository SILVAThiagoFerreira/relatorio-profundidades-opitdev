...
const setFile = selected => {
  file = selected;
  clearResult();
  $('#file-name').textContent = selected ? `${selected.name} · ${(selected.size / 1024).toFixed(0)} KB` : 'Nenhum arquivo selecionado';
};

function presentResult(source, planId, { demo = false } = {}) {
  result = analyzeDxf(source, config);
  runId = new Date().toISOString().replace(/[-:]/g, '').replace('T', '_').slice(0, 15);
  $('#planned').textContent = result.metrics.planned;
  $('#realized').textContent = result.metrics.realized;
  $('#missing').textContent = result.metrics.missing;
  $('#outliers').textContent = result.metrics.outliers;
  $('#adherence').textContent = result.metrics.adherence == null ? '—' : `${(result.metrics.adherence * 100).toFixed(1).replace('.', ',')}%`;
  $('#run-label').textContent = demo ? `${planId} · demonstração` : `Plano ${planId} · ${runId}`;
  $('#demo-note').hidden = !demo;
  $('#rows').replaceChildren(...result.rows.map(row => {
    const tr = document.createElement('tr');
    [row.id, fmt(row.prevista), fmt(row.realizada), row.variacao == null ? '—' : `${row.variacao >= 0 ? '+' : ''}${fmt(row.variacao)}`, row.status].forEach(value => {
...
drop.addEventListener('drop', event => { event.preventDefault(); drop.classList.remove('dragging'); setFile(event.dataTransfer.files[0]); });
...
$('#analyze').addEventListener('click', async () => {
  ...
  try {
    ...
    $('#analyze').textContent = 'Lendo e validando DXF…';
    presentResult(await file.text(), planId);
  } catch (error) {
    $('#error').textContent = error.message || 'Não foi possível ler o arquivo DXF.';
    $('#error').hidden = false;
  } finally {
    $('#analyze').disabled = false;
    $('#analyze').innerHTML = 'Validar e gerar relatório <span>→</span>';
  }
});
...
$('#example').addEventListener('click', async () => {
  ...
  button.textContent = 'Preparando exemplo…';
  try {
    const example = config.outputs.web_example;
    if (!example?.dxf_file || !example?.plan_id) throw new Error('Configuração do exemplo indisponível.');
    const response = await fetch(`${import.meta.env.BASE_URL}${example.dxf_file}`);
    if (!response.ok) throw new Error('Não foi possível carregar o arquivo de exemplo.');
    file = null;
    $('#file').value = '';
    $('#plan-id').value = example.plan_id;
    $('#file-name').textContent = 'Exemplo demonstrativo · dados fictícios';
    presentResult(await response.text(), example.plan_id, { demo: true });
  } catch (error) {
    $('#error').textContent = error.message || 'Não foi possível gerar o exemplo.';
    $('#error').hidden = false;
  } finally {
    button.disabled = false;
    button.textContent = 'Gerar exemplo';
  }
});
...
$('#pdf').addEventListener('click', async () => {
  $('#error').hidden = true;
  try {
    await exportPdf(result, $('#plan-id').value.trim(), runId, config, import.meta.env.BASE_URL);
  } catch (error) {
    $('#error').textContent = error.message || 'Não foi possível gerar o PDF.';
    $('#error').hidden = false;
